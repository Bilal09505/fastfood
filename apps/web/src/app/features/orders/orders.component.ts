...
import type { Order, OrderStatus, MenuItem, Client, User } from '../../core/models';
  ...
  template: `
    <div>
      <div class="page-header">
        ...
        <button *ngIf="auth.isAdmin() || auth.isSalesman()" (click)="openCreate()" class="btn-primary">
          + New Order
        </button>
      </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr *ngFor="let o of filtered()">
                ...
                <td class="text-gray-500">{{ o.items.length }} item(s)</td>
                ...
                <td>
                  <div class="flex gap-1.5 flex-wrap">
                    ...
                    <button *ngIf="auth.isAdmin() && o.status === 'PENDING'"
                            (click)="openAssign(o)" class="btn-ghost text-xs py-1 px-2">
                      Assign
                    </button>
                    <!-- Maker: advance status -->
                    <button *ngIf="auth.isMaker() && canAdvance(o.status)"
                            (click)="advance(o)" class="btn-primary text-xs py-1 px-2">
                      → {{ nextStatus(o.status) }}
                    </button>
                    <!-- Cancel -->
                    <button *ngIf="canCancel(o)"
                            (click)="cancel(o)" class="btn-ghost text-xs py-1 px-2 text-red-600">
                      Cancel
                    </button>
                  </div>
                </td>
              ...
              <tr *ngIf="!filtered().length">
                <td [attr.colspan]="auth.isAdmin() ? 9 : 7" class="text-center text-gray-400 py-8">
                  No orders found
                </td>
              </tr>
            </tbody>
      ...
      <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form [formGroup]="createForm" (ngSubmit)="submitOrder()" class="space-y-4">
          ...
          <div>
            ...
            <textarea formControlName="notes" class="input" rows="2" placeholder="Special instructions..."></textarea>
          </div>

          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="label mb-0">Menu Items</label>
            </div>
            <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-3">
              <div *ngFor="let m of menuItems()" class="flex items-center gap-3">
                <input type="checkbox" [id]="m.id"
                       (change)="toggleItem(m, $event)" class="rounded border-gray-300 text-primary-600" />
                ...
                <input type="number" min="1" value="1"
                       [id]="'qty-' + m.id" class="input w-16 text-center py-1 text-sm" />
              </div>
            </div>
          </div>
          ...
          <div class="flex gap-3">
            <button type="button" (click)="showCreate.set(false)" class="btn-secondary flex-1">Cancel</button>
    ...
    <div *ngIf="showAssign()" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
        ...
        <div class="space-y-2">
          <button *ngFor="let m of makers()" (click)="doAssign(m.id)"
          ...
          <p *ngIf="!makers().length" class="text-sm text-gray-400 text-center py-4">No active makers</p>
        </div>
        <button (click)="showAssign.set(false)" class="btn-secondary w-full mt-4">Cancel</button>
      </div>
    </div>
  `,
})
export class OrdersComponent implements OnInit {
  ...
  submitOrder() {
    ...
    const items = Array.from(this.selectedItems.entries()).map(([menuItemId, quantity]) => {
      const input = document.getElementById('qty-' + menuItemId) as HTMLInputElement;
      return { menuItemId, quantity: input ? parseInt(input.value) || 1 : quantity };
    });
    ...
    this.api.createOrder({ clientId: clientId || undefined, notes, items }).subscribe({
      ...
      error: (e) => { this.orderError.set(e?.error?.message ?? 'Failed'); this.saving.set(false); },
    });
  }
